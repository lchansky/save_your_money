...
from sym_django.settings import BASE_DIR
from .models import *
...
#  3 тест - чтобы при запросе ГЕТ чтобы отрендерился нужный темплейт с нужным контекстом
    ...
    def test_exchanger(self):
        print(BASE_DIR)
        with open(BASE_DIR / 'exchange_rates_pks.json', 'r') as file:
            exchange_rates_pks = json.loads(file.read())
        assert exchanger(1, 2, 1000) == 1000 / exchange_rates_pks['1']['2']
        assert exchanger(2, 5, 5000) == 5000 / exchange_rates_pks['2']['5']
